...
export async function updateOpportunity(req: Request, res: Response) {
    try {
        ...
        // Resources are NEVER re-priced automatically — not on re-estimate, not
        // on the Extended auto-transition, not on any other path. A resource on
        // a deal before a new cost card arrived keeps the rate it was quoted at,
        // permanently. Only a resource added or re-picked AFTER the card changed
        // carries the new rate, because the picker only offers active cards.
        //
        // An earlier version re-priced matched resources whenever the re-estimate
        // counter moved. Removed deliberately: a quote that shifts underneath an
        // estimator who merely moved the deal between stages is a surprise, and
        // the rule is that existing lines stand.
